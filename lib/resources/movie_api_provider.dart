...
class MovieApiProvider {
  ...
  Future<ItemModel> fetchTopRatedMovieList() async {
    //print('entered');
    final response =
        await client.get(Uri.parse("$baseUrl/top_rated?api_key=$apiKey"));
    //print(response.body.toString());
    if (response.statusCode == 200) {
      return ItemModel.fromJson(json.decode(response.body));
    } else {
  ...
  Future<ItemModel> fetchUpcomingMovieList() async {
    //print('entered');
    final response =
        await client.get(Uri.parse("$baseUrl/upcoming?api_key=$apiKey"));
    //print(response.body.toString());
    if (response.statusCode == 200) {
      return ItemModel.fromJson(json.decode(response.body));
    } else {
  ...
  Future<ItemModel> fetchNowPlayingMovieList() async {
    //print('entered');
    final response =
        await client.get(Uri.parse("$baseUrl/now_playing?api_key=$apiKey"));
    //print(response.body.toString());
    if (response.statusCode == 200) {
      return ItemModel.fromJson(json.decode(response.body));
    } else {
  ...
  Future<ItemModel> fetchInterstellar() async {
    ...
    final response = await client.get(Uri.parse(
        "https://api.themoviedb.org/3/search/movie?api_key=4b93d35d5c2089b7716defdd6d32c769&query=interstellar&page=1&include_adult=false"));
    //print(response.body.toString());
    if (response.statusCode == 200) {
      return ItemModel.fromJson(json.decode(response.body));
    } else {
